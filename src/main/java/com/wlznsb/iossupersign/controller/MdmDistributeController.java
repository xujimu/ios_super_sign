...
public class MdmDistributeController {
    ...
    public String getDown(Model model, HttpServletRequest request, HttpServletResponse response, @PathVariable String base64Id) throws JsonProcessingException, UnsupportedEncodingException {
        ...
        String id = String.valueOf(new String(Base64.getDecoder().decode(base64Id.getBytes())));
        log.info("当前id" + id);
        ...
        if(distribute.getApk() != null){
            ...
            distribute.setApk(rootUrl  + "/" + distribute.getAccount() + "/mdmdistribute/" + id + "/" +  id + ".apk?token=" + time);
        }else {
            distribute.setApk("no");
        }
        distribute.setIcon(rootUrl  + "/" + distribute.getAccount() + "/mdmdistribute/" + id + "/" +  id + ".png");
        distribute.setIpa(rootUrl + "/mdmdistribute/" +"getMobile?id=" + id + "&name=" + distribute.getAppName());
...
//        model.addAttribute("downCode", distribute.getDownCode());
        ...
        if(null == distribute.getImages()){
            ...
            model.addAttribute("img1", rootUrl  + "/" + distribute.getAccount() + "/mdmdistribute/" + id + "/" + "img1.png");
            model.addAttribute("img2", rootUrl  + "/" + distribute.getAccount() + "/mdmdistribute/" + id + "/" + "img2.png");
            model.addAttribute("img3", rootUrl  + "/" + distribute.getAccount() + "/mdmdistribute/" + id + "/" + "img3.png");
            model.addAttribute("img4", rootUrl  + "/" + distribute.getAccount() + "/mdmdistribute/" + id + "/" + "img4.png");
        }
        return "mdmdown";
    }
    ...
    public void getUdid(@RequestParam String tempuuid, HttpServletRequest request, HttpServletResponse response) throws IOException {
        String id = tempUuid.get(tempuuid);
        if(id != null){
            ...
            if(null != udid && !udid.equals("")){
                //创建状态
                MdmPackStatusEntity packStatus = new MdmPackStatusEntity(MyUtil.getUuid(), null, null, uuid, udid, null,null,null,new Date(), null, null, "待验证", 1,id,tempContextUrl.replace("http","https"), IpUtils.getIpAddr(request),null,null);
    ...
    public String getDownStatus(Model model, HttpServletRequest request, HttpServletResponse response, @PathVariable String base64Id, @PathVariable String statusId, @PathVariable String uuid, @PathVariable String udid) throws JsonProcessingException, UnsupportedEncodingException {
        ...
        String rootUrl = ServerUtil.getRootUrl(request);
        String id = String.valueOf(new String(Base64.getDecoder().decode(base64Id.getBytes())));
        MdmDistributeEntity distribute = distributeDao.query(String.valueOf(id));
    ...
    @ResponseBody
    @PxCheckLogin(value = false)
    public Map<String,Object> exec(String downCode, @PathVariable String uuid, @PathVariable String id, HttpServletRequest request, @PathVariable String udid) throws IOException {
        Map<String,Object> map = new HashMap<String, Object>();
        try {
            //域名
            log.info("udid" + udid);
